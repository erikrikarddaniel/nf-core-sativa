...
// Convert the flat taxonomy strings into a multifurcating guide tree that
// RAxML-NG will use as a topology constraint (-g / --tree-constraint).
// Corresponds to TaxTreeBuilder in epa_trainer.py.
//
// Input taxonomy TSV format:
//   seq_name <TAB> Kingdom;Phylum;Class;Order;Family;Genus;Species
process SATIVA_TAXONOMY_TREE {
    label 'process_low'
    ...
    // TODO: implement bin/sativa_taxonomy_tree.py
    // Parse semicolon-delimited taxonomy strings → multifurcating Newick tree.
    // Sequences sharing a prefix path are grouped under a common internal node.
    """
    sativa_taxonomy_tree.py \\
        --taxonomy ${taxonomy} \\
        --output   ${prefix}.guide.nwk
...
workflow SATIVA {
    ...
    SATIVA_TAXONOMY_TREE(ch_taxonomy)
    ch_versions = ch_versions.mix(SATIVA_TAXONOMY_TREE.out.versions)
    ...
    def ch_search_input = ch_alignment
        .join(SATIVA_TAXONOMY_TREE.out.guide_tree)
