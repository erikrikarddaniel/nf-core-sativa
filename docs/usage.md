# nf-core/taxmarker: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/taxmarker/usage](https://nf-co.re/taxmarker/usage)

> _Documentation of pipeline parameters is generated automatically from the pipeline schema and can no longer be found in markdown files._

## Introduction

nf-core/taxmarker identifies taxonomically mislabelled sequences by evolutionary placement: it builds a phylogeny from the declared taxonomy, places each sequence back into it after removing it (leave-one-out), and flags sequences whose phylogenetic signal doesn't agree with their declared taxonomy.
...
```bash
nextflow run nf-core/taxmarker --sequences ./sequences.fasta --taxonomy ./taxonomy.tsv --outdir ./results -profile docker
```
...
```bash
nextflow run nf-core/taxmarker -profile docker -params-file params.yaml
```
...
```bash
nextflow pull nf-core/taxmarker
```
...
First, go to the [nf-core/taxmarker releases page](https://github.com/nf-core/taxmarker/releases) and find the latest pipeline version - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
